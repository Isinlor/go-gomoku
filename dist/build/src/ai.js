import { BLACK, EMPTY, WHITE } from './gogomoku.js';
const WIN_SCORE = 1_000_000_000;
const ATTACK_WEIGHTS = [0, 12, 72, 540, 8_000, 500_000];
const DEFENSE_WEIGHTS = [0, 16, 96, 720, 100_000, 500_000];
const EVAL_WEIGHTS = [0, 6, 32, 240, 5_000, WIN_SCORE >> 2];
const LOCAL_LIBERTY_WEIGHTS = [-200, -80, 20, 60, 80];
...
export class GogoAI {
    ...
    moveBuffers = [];
    scoreBuffers = [];
    history = new Int32Array(0);
    candidateMarks = new Uint32Array(0);
    candidateEpoch = 1;
    bufferArea = 0;
    deadline = 0;
    nodesVisited = 0;
    timedOut = false;
    timeoutSignal = new Error('SEARCH_TIMEOUT');
    constructor(options = {}) {
    ...
    findBestMove(position, timeLimitMs) {
    ...
    searchRoot(position, depth, hintMove) {
        this.checkTime(true);
        const moves = this.moveBuffers[0];
        const scores = this.scoreBuffers[0];
    ...
    search(position, depth, alpha, beta, ply) {
    ...
    quiescence(position, alpha, beta, ply, remainingDepth) {
        this.checkTime(false);
        if (position.winner !== EMPTY) {
            return -WIN_SCORE + ply;
        }
    ...
    generateOrderedMoves(position, moves, scores, hintMove, tacticalOnly) {
        if (position.winner !== EMPTY) {
            return 0;
        }
        if (position.stoneCount === 0) {
    ...
    generateFullBoardMoves(position, moves, scores, hintMove, tacticalOnly) {
        let count = 0;
    ...
    scoreMove(position, move, hintMove, tacticalOnly) {
    ...
    insertMove(moves, scores, count, move, score) {
        let index = count;
